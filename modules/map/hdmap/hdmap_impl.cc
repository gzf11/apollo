...
//通过id获得LaneInfo，从lane_table_(map类型获取)
...
//先找lanes,再通过lane找road_id，最后通过id找到road，unordered_set会解决重复id的问题
...
int HDMapImpl::GetNearestLaneWithHeading(
  ...
  std::vector<LaneInfoConstPtr> lanes;
  //首先获取distance距离内方向误差不大于max_heading_difference的所有lane
...
int HDMapImpl::GetLanesWithHeading(const Vec2d& point, const double distance,
  ...
  //通过距离获取所有的lane，KD树是用的线段box，所以可能没有将线段包含进去
  ...
  for (auto& lane : all_lanes) {
    ...
    if (dis <= distance) {
      ...
      //判断航向差是否大于max_heading_difference
...
//获取road边界,以RoadROIBoundaryPtr和JunctionBoundaryPtr表示
int HDMapImpl::GetRoadBoundaries(
  ...
  for (const auto& lane : lanes) {
    ...
    //组合成road_id+section_id
    std::string unique_id = road_id.id() + section_id.id();
    //查看road_section_id_set是否存在该元素
    ...
    //step3、通过road获取junctionid，如果有junctionid则获取junction边界，否则获取road边界
...
//获取road边界，边界用RoadRoiPtr和JunctionInfoConstPtr表示
int HDMapImpl::GetRoadBoundaries(
    const PointENU& point, double radius,
...
//构造roi区域，road边界、路口边界（路口中的road边界不要）、停车位边界
  ...
  for (const auto& road_ptr : roads) {
    ...
    if (road_ptr->has_junction_id()) {
      ...
      if (polygon_id_set.find(junction_ptr->id().id()) ==
          polygon_id_set.end()) {//如果存该junction_id，则将junction信息存入polygons_roi
...
//获取lane前向最近的信号灯
//如果有两个信号与一个停止线相关，则在车道上一定范围内向前移动最近的信号，同时返回两个信号。
int HDMapImpl::GetForwardNearestSignalsOnLane(
  ...
  //点在lane上则将lane存入surrounding_lanes
  ...
  //如果该lane中有信号灯，则获取car_point在lane上的投影点和s值
  ...
  //lane_ptr为空，表示lane上没有信号灯，则搜索相邻车道
  if (lane_ptr == nullptr) {
  ...
  while (lane_ptr != nullptr) {
    ...
    for (const auto& overlap_id : lane_ptr->lane().overlap_id()) {
      ...
      for (int i = 0; i < overlap_ptr->overlap().object_size(); ++i) {
        if (overlap_ptr->overlap().object(i).id().id() == lane_ptr->id().id()) {
          //lane_overlap_info().start_s() 是Object元素与当前lane的重叠区域相对于lane的中心线起点的最小偏移量
          //lane_overlap_offset_s的偏移量是重叠区域的最小偏移距离和当前要搜索的起始s做差值
        ...
        //lane中没有signal_ptr或重叠的最小起始点到搜索范围起始点的前面
        if (signal_ptr == nullptr || lane_overlap_offset_s < 0.0) {
          break;
        }
        //发现离搜索边界距离更短的信号则将min_dist_signal_ptr清空将信号放入
...
//得到一个与原始地图相同的本地地图，只是删除了与给定区域不重叠的所有地图元素。
...
void HDMapImpl::BuildSegmentKDTree(const Table& table,
  ...
  box_table->clear();
  //循环遍历table，将线段做成AABox2d，并将info、segment、id放入box_table
...
//建立KD树分为SegmentKDTree和PolygonKDTree
//lane_segment_boxes_和lane_segment_kdtree_是出参
